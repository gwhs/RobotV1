...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  public static final double MAX_ANGULAR_VELOCITY_RADIANS_PER_SECOND = MAX_VELOCITY_METERS_PER_SECOND /
      Math.hypot(DrivetrainConstants.DRIVETRAIN_TRACKWIDTH_METERS / 2.0,
          DrivetrainConstants.DRIVETRAIN_WHEELBASE_METERS / 2.0); // ~15 RAD/S or 2.3 rotations per sec
  ...
  private TrapezoidProfile.Constraints kThetaControllerConstraints = new TrapezoidProfile.Constraints(Math.PI, Math.PI);
  ...
  public DrivetrainSubsystem() {
    ...
    // m_frontLeftCanCoder = new
    // CANCoder(DrivetrainConstants.FRONT_LEFT_MODULE_STEER_ENCODER);
    // m_frontRightCanCoder = new
    // CANCoder(DrivetrainConstants.FRONT_LEFT_MODULE_STEER_ENCODER);
    // m_backLeftCanCoder = new
    // CANCoder(DrivetrainConstants.BACK_LEFT_MODULE_STEER_ENCODER);
    // m_backRightCanCoder = new
    // CANCoder(DrivetrainConstants.BACK_RIGHT_MODULE_STEER_ENCODER);
  ...
  public void setStates(SwerveModuleState[] states) {
    // System.out.println("states: " + states[FL] + " " + states[FR] + " " +
    // states[BL] + " " + states[BR]);
  ...
  private void setStatesInternal(SwerveModuleState[] states) {// 1.12, 0.95
    m_states = states;
    m_frontLeftModule.set(m_states[FL].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE,
        states[FL].angle.getRadians());
    m_frontRightModule.set(m_states[FR].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE,
        states[FR].angle.getRadians());
    m_backLeftModule.set(m_states[BL].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE,
        states[BL].angle.getRadians());
    m_backRightModule.set(m_states[BR].speedMetersPerSecond * 1.3 / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE,
